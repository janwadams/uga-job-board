  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      ...
      const highEngagement = jobs?.filter(job => {
        const clicks = job.job_link_clicks?.length || 0;
        const views = job.job_views?.length || 0;
        return views > 0 && (clicks / views) > 0.15;
      }).length || 0;

      const mediumEngagement = jobs?.filter(job => {
        const clicks = job.job_link_clicks?.length || 0;
        const views = job.job_views?.length || 0;
        return views > 0 && (clicks / views) >= 0.05 && (clicks / views) <= 0.15;
      }).length || 0;

      const lowEngagement = jobs?.filter(job => {
        const clicks = job.job_link_clicks?.length || 0;
        const views = job.job_views?.length || 0;
        return views > 0 && (clicks / views) < 0.05;
      }).length || 0;
    ...
    <div className="bg-gray-50 min-h-screen p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">Avg Days to Click</h3>
            <p className="text-4xl font-bold text-purple-600 mb-2">{overview.averageDaysToClick}</p>
            <p className="text-xs text-gray-600">after posting</p>
            <p className="text-xs text-gray-500 mt-2 italic">
              How quickly students engage with your posts
            </p>
          </div>
        </div>
