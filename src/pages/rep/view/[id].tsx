// /src/pages/rep/view/[id].tsx
// page for company reps to view their job details (especially archived ones)
...
export default function ViewJobPage() {
  ...
  useEffect(() => {
    // check if user is logged in and is a rep
    const checkAuth = async () => {
      ...
      // check if user is a company rep
    ...
    try {
      const { error } = await supabase
        ...
        .update({ 
          ...
          status: 'pending' // rep jobs go to pending for admin review
        })
        .eq('id', job.id);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {/* status badge - shows if expired, active, pending, etc */}
      <div className="mb-6">
        {isExpired ? (
      ...
      {/* rejection note if the job was rejected by admin */}
      {job.status === 'rejected' && job.rejection_note && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
          <h3 className="text-red-800 font-semibold mb-2">Admin Rejection Note:</h3>
          <p className="text-red-700">{job.rejection_note}</p>
        </div>
      )}
          ...
          {job.status === 'pending' && (
            <div className="md:col-span-2">
              <p className="text-sm text-yellow-600 bg-yellow-50 p-3 rounded">
                This job is pending admin approval. You will be notified once it's reviewed.
              </p>
            </div>
          )}
        </div>
