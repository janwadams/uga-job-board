// pages/rep/edit/[id].tsx - Enhanced version matching create page
...
export default function EditJobPosting() {
  ...
  // Store job status to check if editable
    ...
    const fetchJob = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select('*')
          .eq('id', id)
          .eq('created_by', user.id) // Reps can only edit their own posts
          .single();
        ...
        // Store the original creator and status
        setOriginalCreator(data.created_by);
        setJobStatus(data.status);

        // Check if job is in a state that can be edited
        // Reps can edit pending or rejected jobs (to fix and resubmit)
        // They can also edit active jobs
        if (data.status !== 'pending' && data.status !== 'active' && data.status !== 'rejected') {
          setError('This job posting cannot be edited in its current status.');
          setTimeout(() => router.push('/rep/dashboard'), 2000);
          setLoading(false);
          return;
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If job was rejected, set back to pending when edited
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {/* Show status message if job was rejected */}
      {jobStatus === 'rejected' && (
        <div className="bg-yellow-50 border border-yellow-300 text-yellow-800 p-4 rounded mb-4">
          ℹ️ This job was rejected by admin. Make the necessary changes and submit for review again.
        </div>
      )}
      ...
      {success && (
        <div className="bg-green-100 text-green-800 p-4 rounded mb-4 border border-green-300">
          ✅ Job updated successfully! {jobStatus === 'rejected' ? 'Your posting has been resubmitted for review.' : 'Redirecting to dashboard...'}
        </div>
      )}
