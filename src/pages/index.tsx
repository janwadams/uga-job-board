...
const JobCard = ({ job, isStudent, hasApplied, onApply }: { 
  ...
  // calculate deadline info - job won't be expired but might be urgent
  ...
  return (
    <div className="border bg-white rounded-lg p-6 flex flex-col transition-all duration-300 border-gray-200 hover:shadow-lg hover:border-gray-300">
...
export default function JobBoard() {
  ...
  const fetchJobsWithCreatorInfo = async () => {
    ...
    try {
      ...
      const { data: jobsData, error: jobsError } = await supabase
        ...
        .select('*')
        .eq('status', 'active')
        .gte('deadline', today) // only get jobs with deadline today or in the future
      ...
      // double-check to filter out any expired jobs that might have slipped through
      const activeJobs = enrichedJobs.filter(job => {
  ...
  // get counts - only active (non-expired) jobs
  ...
  // stats for hero section - all are active jobs now
  const stats = useMemo(() => {
    ...
    return { 
      activeJobs: jobs.length, // all jobs are active now
      newJobs, 
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <header className="bg-gradient-to-r from-uga-red to-red-700 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <p className="text-xl opacity-95 mb-6">MIS Career Network - Discover Opportunities at UGA - And Beyond</p>
          
          {/* quick stats - showing only active jobs */}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {!loading && (
          <div className="mb-4 text-sm text-gray-600">
            Showing {paginatedJobs.length} of {finalFilteredJobs.length} active jobs
