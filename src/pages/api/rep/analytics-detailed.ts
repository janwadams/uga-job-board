// pages/api/rep/analytics-detailed.ts
// comprehensive analytics endpoint for the detailed analytics page for company reps
...
export default async function handler(
  ...
  try {
    ...
    // fetch all jobs created by this company rep with link clicks and views
    ...
    if (!jobs || jobs.length === 0) {
      return res.status(200).json({
        repName: `${roleData.first_name || ''} ${roleData.last_name || ''}`.trim() || 'company representative',
