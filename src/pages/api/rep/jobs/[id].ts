// pages/api/rep/jobs/[id].ts
// secure api endpoint to get, update, or delete a specific job for company rep
...
async function handleUpdate(jobId: string, userId: string, updateData: any, res: NextApiResponse) {
  ...
  // if the job was rejected or pending and rep is updating it, set it back to pending for admin review
  if (existingJob.status === 'rejected' || existingJob.status === 'pending') {
