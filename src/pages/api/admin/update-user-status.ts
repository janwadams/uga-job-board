...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Step 1: Ban/un-ban the user in the auth system
    ...
    // Step 2: Update the is_active flag in your UI table
    ...
    // Step 3: Audit the action
    const actionTaken = isActive ? 'enabled' : 'disabled';
    await supabaseAdmin.from('user_status_audit').insert({
      user_id: userId,
      action: actionTaken,
      changed_by_admin_email: session.user.email,
    });

    res.status(200).json({ message: 'User status updated and synced.' });
  } catch (error) {
