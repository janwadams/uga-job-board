...
      password: password,
      email_confirm: true,
      user_metadata: { company_name: company_name }, // The company name is now stored here
    });
...
          is_activ: false, // CHANGED: We are using 'is_activ' to match your table
        },
      ]);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // 2. Insert the user into your custom user_roles table with a pending status.
    // We are no longer trying to insert the company_name here.
    const { error: rolesError } = await supabaseAdmin
      .from('user_roles')
      .insert([
        { 
